...
let rosterSettings={enabled:false,time:'20:00',wakeLead:120};
...
async function loadRoster(){
  ...
  rosterSettings={enabled:!!rosterSettings.enabled,time:rosterSettings.time||'20:00',wakeLead:Number(rosterSettings.wakeLead)||120};
...
$('wakeLead')?.addEventListener('change',()=>{rosterSettings.wakeLead=Number($('wakeLead').value)||120;updateNextDuty()});
$('saveRosterSettings')?.addEventListener('click',()=>{rosterSettings={enabled:$('dutyReminderEnabled').checked,time:$('dutyReminderTime').value||'20:00',wakeLead:Number($('wakeLead').value)||120};localStorage.setItem(rosterSettingsKey,JSON.stringify(rosterSettings));updateNextDuty();rosterMessage('Roster reminder settings saved.')});
...
function showDutyAlert(r,wake,isWake=false){
  ...
  modal.innerHTML=`
    <div class="duty-alert-dialog" role="dialog" aria-modal="true" aria-labelledby="dutyAlertTitle">
      ...
      <div class="duty-alert-wake">⏰ <b>Wake-up: ${esc(wake)}</b><span>${Number(rosterSettings.wakeLead||120)} min before duty</span></div>
