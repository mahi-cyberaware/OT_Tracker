...
function authMessage(text,isError=false){$('authMessage').textContent=text;$('authMessage').className=`message ${isError?'error':'success'}`}
...
$('authForm').onsubmit=async e=>{
  ...
  try{
    if(signUp){
      const r=await sb.auth.signUp({email,password,options:{emailRedirectTo:REDIRECT_URL}});
      ...
      if(r.error){
        ...
        if(/email not confirmed/i.test(msg))authMessage('Your email is not confirmed yet. Switch to Create account, enter the same email, and tap “Resend confirmation”.',true);
        else authMessage(msg,true);
      }else if(r.data.session){user=r.data.user;showApp();await load()}
      else authMessage('Login did not create a session. Please try again.',true);
    }
...
$('resendConfirm').onclick=async()=>{
  ...
  if(!email){authMessage('Enter your email address first.',true);return}
  $('resendConfirm').disabled=true;authMessage('Sending confirmation email…');
  try{
    const r=await sb.auth.resend({type:'signup',email,options:{emailRedirectTo:REDIRECT_URL}});
    if(r.error)authMessage(r.error.message,true);else authMessage('Confirmation email requested. Check your inbox and spam folder.');
  }catch(err){authMessage(err?.message||'Could not resend the email.',true)}
